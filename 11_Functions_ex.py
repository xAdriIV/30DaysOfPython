...
def add_two_numbers(x,z):
    result = x + z 
    return result
print(add_two_numbers(6,6))
...
def check_season(month):
    ...
    if month == 'September' or month == 'October' or month == 'November':
        ...
        ("Error this is not a month, shutdown...")
    return month
...
def quadratic_ecuation(ax,bx,c):
    qec = (ax ** 2) + bx + c
    return qec
print("Quadratatic ecuation: ",quadratic_ecuation(10,10,5))
...
def capitalize_list(my_list_1):
# It takes a list as a parameter and it returns a capitalized list of items
    return [item.capitalize() for item in my_list_1]
    
my_list_1 = ['hello', 'world', 'python']
capitalize_list = capitalize_list(my_list_1)
...
def evens_and_odds(n):
    ...
    for digit in str(n):
        if int(digit) % 2 == 0:
            evens += 1          # The number of odds are 50.
        else:
            odd += 1            # The number of evens are 51.
        return(evens,odds)
...
def factorial(n):
    ...
    for i in range(1, n+1):
        fact *= i
    return fact
print("Factorial de 5: ",factorial(5))    # 5 x 4 x 3 x 2 x 1 = 120
...
def is_same_type(my_list):
    return all(isinstance(item, type(my_list[0])) for item in my_list)
# Write a function which check if provided variable is a valid python variable
def is_valid_python_variable(var):
    return isinstance(var, str) or isinstance(var, int) or isinstance(var, float) or isinstance(var, bool) or isinstance(var, list) or isinstance(var, tuple) or isinstance(var, dict) or isinstance(var, set) or isinstance(var, type(None))
...
def most_spoken_languages():
    language_freq = countries_data(int)
    for country in countries_data:
        language = country.split()[-1]
        language_freq[language] += 1
    return sorted(language_freq, key=language_freq.get, reverse=True)[:10]

# Create a function called the most_populated_countries. It should return 10 or 20 most populated countries in descending order.
def most_populated_countries():
    country_freq = countries_data(int)
    for country in countries_data:
        country_freq[country.split()[-1]] += 1
    return sorted(country_freq, key=country_freq.get, reverse=True)[:10]
# 🎉 CONGRATULATIONS ! 🎉
